 ...
 */
var Bootstrap3ModalForm = Backbone.View.extend({
    ...
    showFormError: function(errorMessage){
        ...
        if (!_alertBox.length){
            _alertBox = $('<div></div>').addClass('alert alert-danger');
        }
    ...
    onModelSyncError: function(model, xhr, options){
        ...
        if (400 == xhr.status){
            ...
            if(jsonResponse['errors']['children'] != undefined) {
                _errors = $.map(jsonResponse['errors']['children'], function(v, i){
                    return v['errors'];
                });
            }
