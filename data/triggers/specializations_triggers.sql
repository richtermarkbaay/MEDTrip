####
# This sql file contains the definitions of triggers involving `specializations` table.
#
...
#
# name: `specializations_ai`
# table: `specializations`
# event: AFTER INSERT
# author: acgvelarde
DROP TRIGGER IF EXISTS specializations_ai $$
CREATE TRIGGER specializations_ai AFTER INSERT on `specializations`
FOR EACH ROW
BEGIN
    ...
    IF termId THEN 
        ...
        INSERT INTO `term_documents` SET `term_id` = termID, `document_id` = NEW.id, `type` = DOCUMENT_TYPE_SPECIALIZATION;
    ELSE
        -- NEW.name does not exist, insert it to terms
        INSERT INTO `terms` SET `name` = NEW.name, `slug`= NEW.slug, `internal` = 1;
        ...
        INSERT INTO `term_documents` (`term_id`, `document_id`, `type`)
            SELECT a.`id`, NEW.id, DOCUMENT_TYPE_SPECIALIZATION FROM `terms` a WHERE a.`name` = NEW.name LIMIT 1;
    END IF;
    
END; $$
### end specializations_ai trigger definition

##----------------------
# AFTER UPDATE specialization trigger
#
# name: specializations_au
# table: `specializations`
# event: AFTER UPDATE
# author: acgvelarde
DROP TRIGGER IF EXISTS specializations_au $$
CREATE TRIGGER specializations_au AFTER UPDATE on `specializations`
FOR EACH ROW
BEGIN
    DECLARE DOCUMENT_TYPE_SPECIALIZATION tinyint(3);
        ...
        IF uniqueTermDocuments > 0 THEN
            ...
            IF uniqueTermDocuments = 1 THEN
                IF termId THEN
                    ...
                    AND `term_documents`.`type` = DOCUMENT_TYPE_SPECIALIZATION;
...
END; $$
## end of specializations_au definition 
##----------------------


##----------------------
# DELETE specialization trigger. Delete terms that are linked to 
#
# name: `specializations_ad`
# table: `specializations`
# event: AFTER DELETE
# author: acgvelarde
DROP TRIGGER IF EXISTS specializations_ad $$
CREATE TRIGGER specializations_ad AFTER DELETE on `specializations`
FOR EACH ROW
BEGIN
    DECLARE DOCUMENT_TYPE_SPECIALIZATION tinyint(3);
    ...
    DELETE FROM `term_documents` WHERE `type` = DOCUMENT_TYPE_SPECIALIZATION AND `document_id` = OLD.id;
