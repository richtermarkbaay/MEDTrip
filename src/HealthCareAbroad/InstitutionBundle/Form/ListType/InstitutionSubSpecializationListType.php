...
class InstitutionSubSpecializationListType extends AbstractType
{
	private $institution;
    	...
    	foreach($this->institution->getInstitutionMedicalCenters() as $center) {
    	    ...
    	    foreach($center->getInstitutionSpecializations() as $institutionSpecialization) {
    	        ...
    	        foreach($institutionSpecialization->getTreatments() as $treatment) {
    	            ...
    	            foreach($treatment->getSubSpecializations() as $subSpecialization) {
    	                $choices[$institutionSpecialization->getSpecialization()->getName()][$subSpecialization->getId()] = $subSpecialization->getName();    	                
    	            }
    	        }
    	    }
    	}
